...
const SelectList = (props) => {
     ...
     useEffect(() => {
        let selectedQuestion=db.find(item => item.title===currentClient).codes.find(item => item.title === currentQuestion).question;
    ...
    return(
        <div className={Styles.dropDown}>
            ...
            <Dropdown options={db.find(item => item.title===currentClient).codes} value={currentQuestion} onChange={questionChangeHandler}/>
        </div>
    );
}
