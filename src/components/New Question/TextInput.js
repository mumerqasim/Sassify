    ...
    return(
        <div className={Styles.TextInputGroup + ' ' + Styles[props.groupClass]}>
            ...
            {props.category === 'custom' ? 
                <div className={expand ? Styles.Modal: Styles.Editor}><span onClick={expandClickedHandler} className={Styles.Expand}>{expand ? 'Minimize':'Expand'}</span><Editor onChange={handleEditorChange} value={props.value} width="100%" height='100%' defaultLanguage="sas" id={props.category} name={props.category}/></div>:
